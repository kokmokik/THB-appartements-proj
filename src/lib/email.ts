...
interface RentalInquiryEmailData {
  name: string;
  email: string;
  propertyName: string;
  moveInDate: Date;
...
export async function sendRentalInquiry(data: RentalInquiryEmailData) {
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Mietanfrage erhalten</title>
</head>
<body style="margin:0;padding:0;background:#f5f5f0;font-family:Georgia,serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f0;padding:40px 0;">
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;max-width:600px;width:100%;">
          ...
          <tr>
            <td style="background:#1a1a2e;padding:36px 40px;text-align:center;">
              <p style="margin:0;color:#c9a96e;font-size:13px;letter-spacing:3px;text-transform:uppercase;">THB Appartements</p>
              <h1 style="margin:8px 0 0;color:#ffffff;font-size:26px;font-weight:normal;">Mietanfrage erhalten</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:36px 40px 24px;">
              <p style="margin:0;font-size:16px;color:#333;line-height:1.6;">
                Sehr geehrte/r ${escapeHtml(data.name)},
              </p>
              <p style="margin:16px 0 0;font-size:15px;color:#555;line-height:1.7;">
                vielen Dank für Ihre Mietanfrage. Wir haben Ihre Anfrage erhalten und werden uns
                in Kürze mit Ihnen in Verbindung setzen, um alle Details zu besprechen und den
          ...
          <tr>
            <td style="padding:28px 40px;">
              <p style="margin:0 0 20px;font-size:13px;letter-spacing:2px;text-transform:uppercase;color:#c9a96e;">Ihre Anfrage im Überblick</p>
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;">
                    <span style="color:#888;font-size:13px;">Apartment</span>
                    <p style="margin:4px 0 0;font-size:15px;color:#1a1a2e;font-weight:bold;">${escapeHtml(data.propertyName)}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;">
                    <table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="50%">
                          <span style="color:#888;font-size:13px;">Gewünschter Einzug</span>
                          <p style="margin:4px 0 0;font-size:15px;color:#1a1a2e;">${formatDate(data.moveInDate)}</p>
                        </td>
                        <td width="50%">
                          <span style="color:#888;font-size:13px;">Mietdauer</span>
                          <p style="margin:4px 0 0;font-size:15px;color:#1a1a2e;">${escapeHtml(durationLabel)}</p>
                        </td>
                      </tr>
                    </table>
                  </td>
                ...
                <tr>
                  <td style="padding:10px 0;${data.message ? "border-bottom:1px solid #f0f0f0;" : ""}">
                    <span style="color:#888;font-size:13px;">Personen im Haushalt</span>
                    <p style="margin:4px 0 0;font-size:15px;color:#1a1a2e;">${data.occupants} ${data.occupants === 1 ? "Person" : "Personen"}</p>
                  </td>
                </tr>
                ${data.message ? `
                <tr>
                  <td style="padding:10px 0;">
                    <span style="color:#888;font-size:13px;">Nachricht</span>
                    <p style="margin:4px 0 0;font-size:14px;color:#555;line-height:1.6;">${escapeHtml(data.message)}</p>
                  </td>
                </tr>
                ` : ""}
              </table>
          ...
          <tr>
            <td style="padding:0 40px 28px;">
              <div style="background:#f8f8f6;border-radius:8px;padding:14px 18px;">
                <span style="font-size:12px;color:#888;">Anfragenummer</span>
                <p style="margin:4px 0 0;font-size:12px;font-family:monospace;color:#555;word-break:break-all;">${data.inquiryId}</p>
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 40px 36px;">
              <p style="margin:0;font-size:14px;color:#555;line-height:1.7;">
                Wir werden uns in Kürze bei Ihnen melden, um alle weiteren Schritte zu besprechen.
  ...
  await getResend().emails.send({
    from: process.env.RESEND_FROM_EMAIL!,
    to: data.email,
    subject: `Mietanfrage erhalten – ${data.propertyName}`,
    html,
  });
}
